...
const cartHidden = document.querySelector(
  "#cartAndFormContainer > section > div.cart__price"
);
...
if (order.length < 1) {
  ...
  fetch(`https://sannier-renaud.fr/portfolio/kanap/api/products`)
        ...
        if (foundArticle != undefined) {
          cartItemSelector.innerHTML += `
                <article class="cart__item" data-id="${
                  foundArticle._id
                }" data-color="${order[h].color}">
                <div class="cart__item__img">
                <img src="${foundArticle.imageUrl}" alt="${
            foundArticle.altTxt
          }">
                </div>
  ...
  btnOrder.addEventListener("click", (e) => {
    ...
    if (testcontactRexExp5 === false) {
      email.style.backgroundColor = colorFalse;
      emailErrorMsg.textContent = "Votre e-mail n'est pas valide";
      checked[4] = false;
    } else {
    ...
    for (let n = 0; n < order.length; n++) {
      idSelectedProduct.push(order[n].id);
    }
    const orderPost = {
      contact: {
    ...
    if (
      ...
      const promise01 = fetch(
        "https://sannier-renaud.fr/portfolio/kanap/api/products/order",
        {
          method: "POST",
